...
public class RelativeTrajectory extends CommandBase 
{
  /** Creates a new DriveThroughTrajectory. */

  /**
   * This entire command just does not work 
   */
  ...
  double alpha;
  public RelativeTrajectory(DriveSubsystem ds, ArrayList<Translation2d> wayPointList, Pose2d end, 
    double maxVelocity, double maxAngularVelocity, double maxAcceleration, double alpha) 
  {
    for (int i = 0; i < wayPointList.size(); i++)
    {
        wayPointList.set(i, new Translation2d((wayPointList.get(i).getX() + drivetrain.getOdometryX()), (wayPointList.get(i).getY() + drivetrain.getOdometryY())));
    }
  ...
  public RelativeTrajectory(DriveSubsystem ds, ArrayList<Pose2d> posePointList, 
    double maxVelocity, double maxAngularVelocity, double maxAcceleration, double alpha) 
  {
    // Use addRequirements() here to declare subsystem dependencies.
    drivetrain = ds;
    ...
    posePoints.add(drivetrain.getOdometry());
